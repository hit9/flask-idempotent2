...
class Idempotent(object):
    ...
    def lock(self, timeout, lock_class=None, block_timeout=None, keyfunc=None):
        ...
        :param block_timeout: Max blocking or wait time in seconds. Defaults to
           ``None``, which indicates continue to try forever.
        :param keyfunc: `keyfunc` to distinguish requests. Which can be
           generated by `gen_keyfunc`. If passed, it will be used instead of
           `default_keyfunc`.
    ...
    def set_response_cache(self, idempotent_id, response, timeout):
        ...
        for instance_str in instances:
            self.redis.setex(self.format_cache_redis_key(instance_str), timeout,
                             idempotent_id)
    ...
    def wrap_lock(self, timeout, lock_class=None, block_timeout=None, keyfunc=None):
